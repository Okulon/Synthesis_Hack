 ...
 *
 * - If `config/local/cycle-clock.json` is missing, pins genesis once (same as `cycle:clock-init`).
...
import { cycleClockStatePath, initCycleClockState } from "./lib/cycleClock.mjs";
import "./lib/env.mjs";
...
function ensureClock() {
  if (!fs.existsSync(cycleClockStatePath())) {
    console.log("[cycle-daemon] No cycle clock — initializing (same as cycle:clock-init)…");
    const s = initCycleClockState();
    console.log("[cycle-daemon] genesisUnix=%s durationSec=%s", s.genesisUnix, s.durationSec);
  }
}
...
function tick() {
  ...
  r = runScript("src/votes-export-frontend.mjs");
